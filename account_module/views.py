...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            user_national_code = register_form.cleaned_data.get('national_code')
            user_mobile = register_form.cleaned_data.get('mobile')
            user_address = register_form.cleaned_data.get('address')
            user_first_name = register_form.cleaned_data.get('first_name')  # دریافت نام
            user_last_name = register_form.cleaned_data.get('last_name')
            user_exists = User.objects.filter(email__iexact=user_email).exists()
            mobile_exists = User.objects.filter(mobile=user_mobile).exists()
            if user_exists:
                register_form.add_error('email', 'ایمیل وارد شده تکراری می باشد')
            elif mobile_exists:
                register_form.add_error('mobile', 'شماره تلفن وارد شده تکراری می باشد')
            else:
                new_user = User(
                    first_name=user_first_name,
                    last_name=user_last_name,
                    national_code=user_national_code,
                    email=user_email,
