...
class ProductListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        db_max_price = product.price if product else 0

        context['db_max_price'] = db_max_price
        context['start_price'] = self.request.GET.get('start_price', 0)
        context['end_price'] = self.request.GET.get('end_price', db_max_price)
...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not ProductVisit.objects.filter(ip__iexact=user_ip, product_id=loaded_product.id).exists():
...
def product_brands_component(request: HttpRequest):
    ...
    return render(request, 'product_module/components/product_brands_component.html', {'brands': product_brands})
